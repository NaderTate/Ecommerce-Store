...
async function page({ searchParams }: any) {
  ...
  const { categories } = await getCategories(sk, 20);
      ...
      <div className="flex flex-col min-h-[90vh]">
        ...
        {pages && Arr && (
          <ol className="flex justify-center gap-1 mt-16 text-sm font-medium">
            <li>
              <a
                href={`/categories?page=${pages.at(0)}&search=${search}`}
            ...
            {Arr &&
              Arr.map((page: any) => (
                <li key={page}>
                  <Link
                    href={`/categories?page=${page}&search=${search}`}
                    className="inline-flex items-center justify-center w-8 h-8 border border-gray-100 rounded-full hover:bg-slate-400/50 transition"
                  >
                    {page}
                  </Link>
            ...
            <li>
              <Link
                href={`/categories?page=${pages.at(-1)}&search=${search}`}
                className="inline-flex items-center justify-center w-8 h-8 border border-gray-100 rounded-full hover:bg-slate-400/50 transition"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
